...
async def test_get_user(client: AsyncClient, admin_token_headers: dict[str, str]):
    ...
    res = await client.get(f"/api/v1/users/{user_id}", headers=admin_token_headers)
...
async def test_create_user(client: AsyncClient):
    response: Response = await client.post(
        "/api/v1/users", json={"username": "user3", "password": "user3"}
    )
...
@pytest.mark.asyncio
async def test_update_user_as_normal_on_self(
    client: AsyncClient, normal_user_token_headers: dict[str, str], users: list[dict]
):
    user_id = users[1]["id"]
    ...
    res = await client.patch(
        ...
        json={"username": "bla", "password": "bla"},
    )
    assert res.status_code == 200
    ...
    res = await client.patch(
        f"/api/v1/users/{user_id}",
        headers=normal_user_token_headers,
        json={"username": "normal_user", "password": "normal_user"},
    )
    assert res.status_code == 200


@pytest.mark.asyncio
async def test_update_user_as_normal_on_other(
    client: AsyncClient, normal_user_token_headers: dict[str, str], users: list[dict]
):
    user_id = users[0]["id"]
...
@pytest.mark.asyncio
async def test_update_user_as_admin_on_other(
    client: AsyncClient,
    ...
    assert res.status_code == 200, "Unable to edit user"
    ...
    res = await client.patch(
        f"/api/v1/users/{user_id}",
        headers=admin_token_headers,
        json={"username": "normal_user_3", "password": "normal_user_3"},
    )
    assert res.status_code == 404
...
async def test_delete_user_as_normal_on_self(client: AsyncClient, users: list[dict]):
    login_data = {"username": "user3", "password": "user3"}
...
@pytest.mark.asyncio
async def test_delete_user_as_normal_on_other(
    client: AsyncClient, normal_user_token_headers: dict[str, str], users: list[dict]
):
    user_id = users[0]["id"]
...
@pytest.mark.asyncio
async def test_delete_user_as_admin_on_other(
    client: AsyncClient, admin_token_headers: dict[str, str]
):
    response: Response = await client.post(
        "/api/v1/users", json={"username": "user3", "password": "user3"}
    ...
    res = await client.delete(f"/api/v1/users/{user_id}", headers=admin_token_headers)
    assert res.status_code == 204
...
async def test_path_user_id(client: AsyncClient, admin_token_headers):
    ...
    res = await client.get("/api/v1/users/_", headers=admin_token_headers)
    assert res.status_code == 400


@pytest.mark.asyncio
async def test_bad_hash_id(client: AsyncClient, admin_token_headers: dict[str, str]):
    response = await client.get("/api/v1/users/abc", headers=admin_token_headers)
